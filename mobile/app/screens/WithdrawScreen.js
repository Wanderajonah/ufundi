...
export default function WithdrawScreen({ onNavigate, userRole = 'customer' }) {
  ...
  const handleWithdraw = async () => {
    if (numericAmount <= 0) return Alert.alert(t('Enter amount'), t('Please enter the amount you want to withdraw.'));
    if (numericAmount > (wallet?.balance || 0)) return Alert.alert(t('Insufficient balance'), t('You do not have enough funds for this withdrawal.'));
    ...
    setLoading(true);
    try {
      await withdraw(numericAmount, method === 'mtn' ? 'mtn_momo' : 'airtel_money', `+256${cleanPhone}`);
      Alert.alert(
        t('Withdrawal Successful'),
        t('{{amount}} has been sent to your {{method}} wallet.', {
      ...
      Alert.alert(t('Withdrawal failed'), error.response?.data?.message || t('Something went wrong. Please try again.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const content = (
    <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
      ...
      <Text style={[styles.hint, isFundi && styles.fundiHint]}>
        {t('Funds will be sent to your {{method}} wallet.', {
      ...
      <PrimaryButton
        ...
        loading={loading}
        disabled={numericAmount <= 0 || numericAmount > (wallet?.balance || 0)}
      >
        {numericAmount > 0 ? t('Withdraw {{amount}}', { amount: formatAmount(numericAmount) }) : t('Enter an amount')}
      </PrimaryButton>
    </ScrollView>
  );
